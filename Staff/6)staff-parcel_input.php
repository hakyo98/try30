...
<div class="types">
<?php
echo '<a href="6)staff-parcel_input.php" class="button selected">Input</a>';
echo '<a href="6.2)staff-parcel_search.php" class="button">Search</a>';
?>
</div>
...
<?php
$q="SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'golf_view' AND TABLE_NAME = 'packages'";
$result=$mysqli->query($q);
if(!$result){
	echo $mysqli->connect_errno.": ".$mysqli->connect_error;
}
$row=$result->fetch_array();
$gvpackageno=$row['AUTO_INCREMENT'];
if(isset($_POST['submit'])) {
$gvpackageno=$gvpackageno+1;}
?>
<div style="text-align:center;">
<h2 style="padding:25px 0px 0px; margin:0px; background-color: #F9F9F9;">Input package record</h2>

  <form style="text-align:center; background-color: #F9F9F9;" action="6)staff-parcel_input.php" method="POST">
	<b>GolfView tag  <?php echo $gvpackageno;?></b>
      <input Class="sign" type="text" placeholder="Delivery company" name="Delivery_company" size="13" required>
      <b>Arrival</b><input Class="sign"  type="date" value="<?php echo date('Y-m-d');?>" name="Date_of_arrival" required>
      <b>Retrieve</b><input Class="sign"  type="date" name="Date_of_retrieve" >
		<select name="type" id="type" style="width: fit-content;">
			<option value="bag">Bag</option>
			<option value="box">Box</option>
		</select>
      <input Class="sign" type="text" placeholder="Name" name="Name" size="25" required>
	  <input Class="sign" type="text" placeholder="National id" name="National_id" size="10">
	  <input Class="sign" type="text" placeholder="Staff id" name="poic" size="6" required>
	  <input style="font-size: 17px;" class="button1" type="submit" name="submit" value="Submit"><br>
  </form>
</div>
...
<div style="text-align:center; background-color: #F9F9F9;">
	...
	<?php 
		if(isset($_POST['submit'])) {
			...
			$q="INSERT INTO packages (gvpackageno,delivery_partner,doa,dor,type_of_package,owner_name,citizen_id,poic)
			VALUES ('$gvpackageno','$delivery_partner','$doa','$dor','$type_of_package','$owner_name','$citizen_id','$poic')";
			$result=$mysqli->query($q);
			if(!$result){
				echo $mysqli->connect_errno.": ".$mysqli->connect_error;
			}
	...
	<table style="text-align:center; margin: auto; padding: 0px 0px 10px;">
		...
		?>
         ...
         <tr style="padding:5px; background-color: <?php echo $color;?>;">
			...
			<?php
			if($row['citizen_id']!=""){$citizen_id=$row['citizen_id'];}
			else{$citizen_id='-';}
			?>
			
            <td><?php echo $citizen_id;?></td>
            <td><?php echo $row['poic'];?></td>

			<form action="6)staff-parcel_input.php" method="post">
			<input type="hidden" name="gvpackageno" value="<?php echo $row['gvpackageno'];?>">
