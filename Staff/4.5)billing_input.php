...
<div class="types">
<?php
echo '<a href="4.5)billing_input.php" class="button selected">Input</a>';
echo '<a href="4.6)billing_search.php" class="button">Search</a>';
?>
</div>
...
<h2 style="padding: 10px 0px 10px; margin:0px;">Input bill</h2>
  <form action="4.5)billing_input.php" method="POST">
      <b>Bill No.  <?php echo $bill_id;?></b>
      <input Class="main" type="text" placeholder="Room No" name="roomno" size="7" required>
      <input Class="main" type="text" placeholder="Electricity" name="Electricity_bill" size="7" required>
      <input Class="main" type="text" placeholder="Water" name="Water_bill" size="7" required>
      <b>Wifi</b><input Class="main" type="checkbox" name="Wificable_fee">
      <input Class="main" type="text" placeholder="Maintenance" name="maintenance_fee" size="10" required>
	  <b>Due date</b><input Class="main"  type="date" value="<?php $date = date('Y-m-05', strtotime( date('Y-m-d')." +1 month")); echo $date; ?>" name="due_date" required>
...
if(isset($_POST['submit'])) {
	...
	$roomno=$_POST['roomno'];
	$Electricity_bill=$_POST['Electricity_bill']*6;
	$Water_bill=$_POST['Water_bill']*2;
	...
	$q="SELECT lease.Lease_id FROM lease WHERE lease.status='Active' AND lease.roomno='$roomno'";
	$result=$mysqli->query($q);
	if(!$result){
		echo $mysqli->connect_errno.": ".$mysqli->connect_error;
	}
	...
	$q="INSERT INTO utility (bill_id,roomno,Electricity_bill,Water_bill,Wificable_fee,maintenance_fee,Total_room_fee,Lease_id,due_date)
	VALUES ('','$roomno','$Electricity_bill','$Water_bill','$Wificable_fee','$maintenance_fee','$Total_room_fee','$lease_id','$due_date')";
	$result=$mysqli->query($q);
	if(!$result){
		echo $mysqli->connect_errno.": ".$mysqli->connect_error;
	}
