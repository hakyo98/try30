...
echo '<a href="5.2)staff-maintenance_input.php" class="button selected">Input</a>';
?>
</div>

<?php
$q="SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'golf_view' AND TABLE_NAME = 'maintenance'";
$result=$mysqli->query($q);
if(!$result){
	echo $mysqli->connect_errno.": ".$mysqli->connect_error;
}
$row=$result->fetch_array();
$maintenance_no=$row['AUTO_INCREMENT'];
$maintenance_n=$row['AUTO_INCREMENT'];
if(isset($_POST['submit'])) {
$maintenance_n=$maintenance_no+1;}
?>
<!--%%%%% Form for Request %%%%-->
<div style="text-align:center; background-color: #F9F9F9; margin: auto;">
<h2 style="padding: 25px 0px 25px; margin:0px;">Input maintenance request</h2>
  <form action="5.2)staff-maintenance_input.php" method="POST">
      <b>Request No.  <?php echo $maintenance_n;?></b>
      <input Class="main" type="text" placeholder="Room No" name="roomno" size="7" required>
      <b>Request date</b><input Class="main"  type="date" value="<?php echo date('Y-m-d');?>" name="request_date" required>
      <input Class="main" type="text" placeholder="Description" name="description" size="100" required><br>
      <b>Maintenance date</b><input Class="main"  type="date" value="<?php echo date('Y-m-d');?>" name="dateom" required>
	  <b>time</b><input Class="main" type="time" name="timeom" min="09:00" max="17:30" required>
      <input Class="main" type="text" placeholder="Telephone" name="telephone" size="15" required>
	  <input Class="main" type="text" placeholder="Record staff" name="oic" size="10" required>
...
if(isset($_POST['submit'])) {
	...
	$q="INSERT INTO maintenance (maintenance_no,roomno,request_date,description,dateom,timeom,telephone,oic,mic)
	VALUES ('$maintenance_no','$roomno','$request_date','$description','$dateom','$timeom','$telephone','$oic','$mic')";
	$result=$mysqli->query($q);
	if(!$result){
		echo $mysqli->connect_errno.": ".$mysqli->connect_error;
	}
